...
const replaceDollarsToUsdc = (amount: string) =>
  amount.includes("$") ? `${amount.replace("$", "")} USDC` : amount;
...
function formatNumber({
  ...
  return (
    (prefix ?? "") +
    replaceDollarsToUsdc(
      new Intl.NumberFormat(locale, formatterOptions).format(
        hardCodedInputValue
      )
    )
  );
...
export function useFormatter() {
  ...
  const formatPriceImpactWithLocales = useCallback(
    (priceImpact: Percent | undefined) =>
      formatPriceImpact(priceImpact, formatterLocale),
    [formatterLocale]
  );
  ...
  const formatSlippageWithLocales = useCallback(
    (slippage: Percent | undefined) =>
      formatSlippage(slippage, formatterLocale),
    [formatterLocale]
  );
