...
import { BigintIsh, ChainId, CurrencyAmount, Token, TradeType } from '@vanadex/sdk-core'
...
export function getRouter(chainId: ChainId, web3Provider: Web3Provider | undefined): RouterAndProvider {
  ...
  if (
    ...
    console.log('cached provider router')
  ...
  if (providerChainId !== undefined && chainId === providerChainId && web3Provider !== undefined) {
    ...
    console.log('new provider router')
    return cachedProviderRouter.routerProvider
  }
...
async function getQuote(
  ...
  const swapRoute = await router.route(amount, quoteCurrency, tradeType, undefined, routerConfig)

  console.log('swapRoute', swapRoute)
  if (!swapRoute) {
    console.log('swapRoute not found')
