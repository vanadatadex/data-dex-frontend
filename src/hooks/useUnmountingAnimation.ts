...
export function useUnmountingAnimation(
  ...
  useEffect(() => {
    ...
    const animated = animatedElements?.map((element) => element.current) ?? [
      current,
    ];
    ...
    parent.removeChild = function <T extends Node>(child: T) {
      ...
      if ((child as Node) === current && animated) {
        ...
        animated.forEach((element) =>
          element?.classList.add(getAnimatingClass())
        );

        // Check if any of the animated elements is animating
        const animating = animated.find((element) =>
          isAnimating(element ?? undefined)
        );
        if (animating) {
