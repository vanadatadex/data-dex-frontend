import { ChainId, Token } from "@vanadex/sdk-core";
...
async function getColorFromToken(
  ...
  try {
    if (primarySrc) {
      ...
      color =
        colorArray === DEFAULT_COLOR
          ? null
          : convertColorArrayToString(colorArray);
    }
    ...
    console.warn(
      `Unable to load logoURI (${token.symbol}): ${primarySrc}, ${wrappedToken.logoURI}`
    );
    return null;
  }
}
