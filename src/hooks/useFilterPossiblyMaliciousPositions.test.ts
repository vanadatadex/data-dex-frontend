...
const mockUseTokenContractsConstant =
  useTokenContractsConstant as jest.MockedFunction<
    typeof useTokenContractsConstant
  >;
...
const positions: PositionDetails[] = [
  {
    ...
    feeGrowthInside1LastX128: BigNumber.from(
      "0x9691f41769e1a6a196e8f5bcddf89c"
    ),
...
describe("useFilterPossiblyMaliciousPositions", () => {
  it("filters out unsafe positions", async () => {
    ...
    const { result } = renderHook(() =>
      useFilterPossiblyMaliciousPositions(positions)
    );

    expect(
      result.current.some(
        (position) =>
          position.token1 === "0xB85C51F89788C1B3Bd4568f773e8FfB40cA587Bb"
      )
    ).toEqual(false);
  });
  it("checks the chain for addresses not on the token list", async () => {
    ...
    expect(mockUseTokenContractsConstant).toHaveBeenCalledWith(
      [
        ...
        "0xB85C51F89788C1B3Bd4568f773e8FfB40cA587Bb",
      ],
      "symbol"
    );
  });
