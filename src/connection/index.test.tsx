...
describe("connection utility/metadata tests", () => {
  ...
  it("Phantom Wallet Injected Desktop displays as MetaMask", async () => {
    ...
    const { displayed, injected } = createWalletEnvironment({
      isMetaMask: true,
    }); // Phantom sets isMetaMask to true
  ...
  it("Generic Browser Wallet that injects as MetaMask", async () => {
    const { displayed, injected } =
      createWalletEnvironment(UNKNOWN_MM_INJECTOR);
  ...
  it("Generic Unknown Injected Wallet Browser", async () => {
    ...
    expect(injected.getIcon?.(/* isDarkMode= */ false)).toBe(
      INJECTED_LIGHT_ICON
    );
  ...
  it("Coinbase Mobile Browser", async () => {
    const { displayed, coinbase } = createWalletEnvironment(
      { isCoinbaseWallet: true },
      true
    );
