...
it("Should call activate function on a connection", async () => {
  ...
  renderHook(() =>
    useAppDispatch()(updateSelectedWallet({ wallet: ConnectionType.INJECTED }))
  );
  const initialSelectedWallet = renderHook(() =>
    useAppSelector((state) => state.user.selectedWallet)
  );
  ...
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection activating: ${mockConnection.getName()}`
  );
  expect(onSuccess).toHaveBeenCalledTimes(0);
  const pendingSelectedWallet = renderHook(() =>
    useAppSelector((state) => state.user.selectedWallet)
  );
  ...
  expect(mockConnection.connector.activate).toHaveBeenCalledTimes(1);
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection activated: ${mockConnection.getName()}`
  );
  expect(console.debug).toHaveBeenCalledTimes(2);
  expect(onSuccess).toHaveBeenCalledTimes(1);
  const finalSelectedWallet = renderHook(() =>
    useAppSelector((state) => state.user.selectedWallet)
  );
...
it("Should properly display error state", async () => {
  ...
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection failed: ${mockConnection.getName()}`
  );
  expect(console.debug).toHaveBeenCalledTimes(2);
  expect(mockConnection.connector.activate).toHaveBeenCalledTimes(1);
  expect(onSuccess).toHaveBeenCalledTimes(0);
