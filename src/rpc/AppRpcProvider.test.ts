...
describe("AppRpcProvider", () => {
  ...
  test("constructor initializes with valid providers", () => {
    expect(
      () => new AppRpcProvider(ChainId.MAINNET, mockProviders)
    ).not.toThrow();
  });

  test("constructor throws with empty providers array", () => {
    expect(() => new AppRpcProvider(ChainId.MAINNET, [])).toThrow(
  ...
  test("constructor throws with network mismatch", () => {
    ...
    expect(() => new AppRpcProvider(ChainId.MAINNET, mockProviders)).toThrow(
      "networks mismatch"
    );
  });
  ...
  test("constructor throws with invalid providers", () => {
    ...
    expect(
      () => new AppRpcProvider(ChainId.MAINNET, [{} as JsonRpcProvider])
    ).toThrow("invalid provider");
  });

  test("handles sendTransaction", async () => {
    const hash = "0x123";
    mockProvider1.sendTransaction.mockResolvedValue({
      hash,
    } as TransactionResponse);
    const provider = new AppRpcProvider(ChainId.MAINNET, [mockProvider1]);

    const result = await provider.perform("sendTransaction", {
  ...
  test("should sort faster providers before slower providers", async () => {
    ...
    mockProvider1.getBlockNumber = jest.fn(
      () => new Promise((resolve) => setTimeout(() => resolve(1), SLOW))
    );

    const FAST = 1;
    mockProvider2.getBlockNumber = jest.fn(
      () => new Promise((resolve) => setTimeout(() => resolve(1), FAST))
    );

    const appRpcProvider = new AppRpcProvider(ChainId.MAINNET, mockProviders);

    // Evaluate all providers
    const evaluationPromises = appRpcProvider.providerEvaluations.map(
      appRpcProvider.evaluateProvider
    );
    await Promise.all(evaluationPromises);

    // Validate that the providers are sorted correctly by latency
    const [fastProvider, slowProvider] = AppRpcProvider.sortProviders(
      appRpcProvider.providerEvaluations.slice()
    );

    expect(fastProvider.performance.latency).toBeLessThan(
      slowProvider.performance.latency
    );
  });
  ...
  test("should sort failing providers after successful providers", async () => {
    ...
    // Validate that the providers are sorted correctly by latency
    const [provider, failingProvider] = AppRpcProvider.sortProviders(
      appRpcProvider.providerEvaluations.slice()
    );
    expect(provider.performance.failureCount).toBeLessThan(
      failingProvider.performance.failureCount
    );
  });
  ...
  test("should increment failureCount on provider failure", async () => {
    ...
    await appRpcProvider.evaluateProvider(
      appRpcProvider.providerEvaluations[0]
    );

    // Validate that the failureCount was incremented
    expect(appRpcProvider.providerEvaluations[0].performance.failureCount).toBe(
      1
    );
  });
});
