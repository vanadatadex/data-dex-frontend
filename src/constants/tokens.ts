import { ChainId, Currency, Ether, NativeCurrency, Token, UNI_ADDRESSES, WETH9 } from '@vanadex/sdk-core'
...
class VanaNativeCurrency extends NativeCurrency {
  ...
  get wrapped(): Token {
    if (!isVana(this.chainId)) throw new Error('Not vana')
    const wrapped = WRAPPED_NATIVE_CURRENCY[this.chainId]
    invariant(wrapped instanceof Token)
    return wrapped
  }
  ...
  public constructor(chainId: number) {
    if (!isVana(chainId)) throw new Error('Not vana')
    super(chainId, 18, 'VANA', 'VANA')
  }
}
...
export function nativeOnChain(chainId: number): NativeCurrency | Token {
  ...
  } else if (isVana(chainId)) {
    nativeCurrency = new VanaNativeCurrency(chainId)
  } else {
